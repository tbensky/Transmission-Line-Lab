...
This lab involves a good dose of electronics and electromagnetics.  Dust off your PHYS-133 and PHYS-206 knowledge for this one.

A transmission line is two conductors that do not touch for the entire length of the line. The two conductors can be thought of as a 'signal' and 'return.'  Transmission lines are used to transmit signals in the form of electromagnetic energy over some needed distance, for example, between one device and another.  The black cable wiring your house for cable-television or ethernet cables for data are examples transmissions lines.

A coaxial transmission line has its two conductors sharing a common axis of cylindrical symmetry that runs the length of the cable. There is a small diameter inner conductor which resembles a long bare copper wire.  This wrapped in an insulating dielectric, then surrounded by an outer conducting sheath.  The entire assembly is then wrapped in a rubber coating for protection. An example is shown here (from Wikipedia).
...
As we begin, note that the three properties all electrical devices have are resistance, capacitance, and inductance.  In your last introductory physics course, you studied all of these from a DC standpoint. Here, DC means a constant voltage.  Resistance $(R)$ led to Ohms law, $V=IR$, capacitance $(C)$ led to the ability to separate and hold charge $Q=CV$, and inductance $(L)$ is the proportionality constant between magnetic flux and current, or $\Phi_B=LI$.

It is rare for DC voltages to be used with transmission lines. Usually AC voltages are used, as we will do in this lab. Think: sine waves or pulses. This means the frequency of the AC voltage will be a critical parameter.  From an electrical standpoint, resistors behave about the same under AC as they do in DC. 

Capacitors and inductors however, immediately behave wildly different to AC stimulation.  Capacitors present a reactance to AC called ``capacitive reactance'' which is $X_C=1/(2\pi fC)$.  Inductors present an ``inductive reactance'' of $X_L=2\pi L$. Both have units of Ohms, and note that unlike the familiar constant $R$, the reactance of a capacitor and inductor depend on the applied frequency, $f$. Reactances are not linear, ohmic behavior that follow Ohm's law like a resistor does.

Lastly, as we said, all electrical devices have resistance, capacitance, and inductance. They work together to present a ``total resistance'' to an AC signal. This resistance is called ``impedance,'' and is given the symbol $Z$.  All told, the impedance of a device is
...
The theory section includes some derivations. Treat all derivations like ``data'' you took, meaning fully include and present them in your report.

\subsection{Derivations}

To begin, derive from first principles, the capacitance per unit length (${\tilde C}$) and inductance per unit length (${\tilde L}$) a coaxial arrangement of conductors. These are PHYS-133 level problems. You can find help in any book for that level of class. Hints:

\begin{enumerate}

\item For the capacitance per unit length, you can find help by looking up a ``cylindrical capacitor.'' This will involve Gauss's law, $V=-\int {\vec E}\cdot d{\vec l}$, and the definition of capacitance, $C=Q/V$. (See Example 2, p. 746 of \url{https://b-ok.cc/book/5329620/fc9583}).
...
Include your derivations, that should involve a figure of each system (Gaussian surface, integral paths, etc. help the derivation along).
...
Review a two-resistor voltage divider from PHYS-206. The Wikipedia page \url{https://en.wikipedia.org/wiki/Voltage_divider} is ideal. Have the structure and equation for $V_{out}$ as a function of $V_{in}$, $Z_1$ and $Z_2$ handy. Assume $Z_1$ is connected to $+V_{in}$ and $Z_2$ is connected to ground. Note: In PHYS-206, the voltage divider equation was likely written with R's instead of Z's. Z's are used when considering resistance of AC signals and are called ``impedance'' instead of resistance.  All have units of Ohms.
...
Build a voltage divider using a $10$K resistor as $Z_1$ and $Z_2$ as the transmission line itself.

Note here, $Z_1=10,000$ and $Z_2=X_c$ where $X_c$ is the capacitive reactance of the transmission line, which is $X_c=1/(2\pi fC)$. Plug these values/quantities into the equation for a voltage divider and solve for $C$. 

To prepare for data analysis, linearize your equation for $C$, so the slope of a linear fit can be used to find $C$.

Next, place an AC signal across the voltage divider. Monitor the peak to peak voltage on CH1 of your oscilloscope, which is $V_{in}$. On CH2, monitor the peak to peak voltage across the transmission line, which is $V_{out}$. Measure $V_{out}$ vs. $V_{in}$ for a variety of driving frequencies from 1000 - 10,000 Hz. Use this data and your linearized equation for $C$ to perform a curve fit. Use the slope of your fit find $C$. This is the capacitance of the transmission line.


You now have a measured $C$ and a theortical ${\tilde C}$ from your derivation. Compare the two noting that $C$ is capacitance and ${\tilde C}$ is capacitance per unit length.

\subsection{The LC Circuit}

An LRC circuit consists of an inductor (L) and capacitor (C) as shown here.
...
Using Kirchoff's Law for the loop, that $\Sigma V=0$ (around the loop), set up and solve a differential equation that will lead to a formula for the fundamental frequency of such a circuit. Recall for an inductor that $V=-LdI/dt$ and for a capacitor $V=Q/C$.  You should get a fundamental frequency of $\omega_{LC}=1/\sqrt{LC}$. Note also that $\omega$ is in rad/s, convenient for math, but lab equipment measures $f$ in Hz, which is $f=2\pi\omega$.
...
Build an RLC circuit using a capacitor and the transmission line as the $L$. (Here three different capacitors will be used, a $269$ nF, $1\mu$F, and a $10\mu$F.  Put a $50\Omega$ resistor in series with them.  Apply an AC voltage across the whole circuit. Monitor this voltage on CH1 and the voltage just across the resistor on CH2.
...
To analyze this data, plot the CH2 voltage vs frequency.  Since you explored a couple of orders of magnitude in frequency, plot the log (base 10) of frequencies on the $x$ axis, and voltage on the $y$ axis. It should appear like a peaked function that is very wide.  The center of this peak is the resonance frequency. Do a curve fit to find it. It is not clear what function to use. Consider {\sl some function} that is peaked and has a center as a parameter.
...
The hard boundary for the diameter of the inner conductor assumes a high frequency AC signal. Higher frequency currents tend to travel on the surface of conductors, which may very well be right at the diameter you measured.  But lower frequencies will travel at more depths within the conductor.  The ``skin depth'' parameter is a measure of this depth, which is

\begin{equation}
\delta=\sqrt{\frac{2\rho}{\omega \mu_0}},
\end{equation}

where $\rho$ is the resistivity of the conductor, $\omega$ is the radial frequency of the AC signal, and $\mu_0$ is the usual constant. Evaluate this for some central frequency you used in your inductor measurements. Does your result warrant a correction to the diameter of the inner conductor used to compute ${\tilde L}$? If so, by how much, and can this bring ${\tilde L}$ in more agreement with $L$ that you measured?
...
Suppose the rope was 1000 ft long and you only had access to the launch end of it (the other end is too far away).  Do you think it would be possible to determine how fast the pulse travels in the rope with just such access to the launch end? Explain how.
...
The time dependent position of the pulse down the transmission line necessarily means that it has a time varying electric field (or E-field). Any time varying E-field also gives rise to a time varying B-field (and vise versa); this is Farsday's Law.  Supposing the E-field is traveling wave, or $E=E_0\sin(kx-\omega t)$ and the B-field is  $B=B_0\sin(kx-\omega t)$, they are related by

\begin{equation}
-\frac{\partial B}{\partial x}=\mu_0 \epsilon_0 \frac{\partial E}{\partial t}.
\end{equation}

Note the usual definitions of $k=2\pi/\lambda$ and $\omega=2\pi f$.  Plug the functions for $E$ and $B$ into this, and note that for $E_0$ and $B_0$ (in an electromagnetic wave), $E_0=vB_0$, where $v$ is the speed of the wave (in this case, electromagnetic wave). Compute a theoretical value for the speed of the wave.  (Note in physics that $\mu_0=4\pi\times 10^{-7}$ m/A (exactly) and $\epsilon_0$ is experimentally determined. 
...
\item The capacitance of a capacitor is increased by the presence of a dielectric, namely $C_{with-dielectic}=\kappa C_{without-dielectric}$.  So you can now scale your theoretical value of $C$ (derived with no dielectric assumed) of your transmission line to what you determined experimentally and or from the meter.
\end{enumerate}
...
Here's we'd start by putting $Z_3=Z_1+Z_2$ (since they're in series), and arrive at Fig.~\ref{z04}.  Then, we'd see that $Z_2$ and $Z_3$ are in parallel, so we could reduce this to Fig.~\ref{z05} with $Z_4=1/(1/Z_2+1/Z_3)$ and finally to Fig.~\ref{z06}, with $Z_5=Z_1+Z_4$. So the impedance across $a$ and $b$ in Fig~\ref{z06} is the same as that in the original circuit.
...
With that circuit analysis review done, let's apply it to a transmission line. A full transmission line can be modeled as series of circuit units like that shown in Fig.~\ref{z01}.  When many such units are strung together, we'd get the circuit in Fig.~\ref{linezs}. What would the impedance be of such a chain?
...
Suppose the infinite chain of circuit units in Fig.~\ref{linezs} had a net impedance of $Z_0$ as shown. In other words, if we carefully broke down each circuit init as done in the warm-up, we'd get some equivalent impedance $Z_0$.  How would this be done for an an infinite chain? 
...
\begin{equation}
Z_{total}=Z_1+\frac{1}{\frac{1}{Z_2}+\frac{1}{Z_0}},
\end{equation}
...
Now, typically a transmission line is represented by inductors as the series elements and capacitors as the parallel elements as shown in Fig.~\ref{tline}.  Thus, for Eqn.~\ref{z0_feyn}, $Z_1=\omega L$ and $Z_2=1/(\omega C)$. The $\omega L/2$ term in front is a bit inconvenient and may be swept away by claiming that each series inductor is has a value of $L/2$ or $L\rightarrow L/2$, giving 
...
Evaluate this equation at the parameters for $b$ and $a$ that you have.  

Note in this equation that $1/\epsilon_0 c$ has the dimensions of resistance (Ohms) and is equal to $120\pi$. The $\ln$ dependence on the geometrical parameters is weak, so most coaxial lines have an $Z_0$ in the range of $50$ to a few hundred Ohms (see Feynman, Sect. 24-1).
...
Anything connected to the far end of the transmission line is called the ``load.'' Loads are electrical, so will have an impedance. Let's call the load $Z_L$.  In the present case, the load is infinite (nothing is connected to it).  See what happens to the reflection when $Z_L=0$ (go ahead and connect the two conductors at the far end). The pulse should become inverted, just like in the simulation when the end of the rope was clamped.

What about $Z_L$ in between $0$ and $\infty$?  Let's connect a variable resistor to the end of the transmission line and vary $Z_L$ and see what happens to the reflection. You can see that the amplitude of the reflected pulse varies with $Z_L$. As $Z_L$ is decreased, the amplitude of the reflected pulse decreases as well. The ratio of the reflected pulse amplitude to that of the input pulse amplitude is $\Gamma$, and is called the ``reflection coefficient.'' It is given by

\begin{equation}
\Gamma=\frac{V_{reflected}}{V_{input}}.
\end{equation}
...
In one more use of Eqn.~\ref{gamma}, what do you think happens to the reflection if $Z_L=Z_0$, that is, when the load resistance equals the impedance of the cable?  Try this setting and observe the reflected pulse amplitude on the oscilloscope.
...
\subsection{Derivation: Potential relationship between $Z_L$ vs. $Z_0$}

In the last section, there was some indication that setting $Z_L=Z_0$ leads to cancellation of the reflected pulse.  Think now, if there is no reflection, where does all of the initial pulse energy go? (Try to answer this before continuing.)
...
Here, $Z_0$ is the transmission line and $Z_L$ is the load connected to the far end of it.  Compute the power ($P=I^2Z$) flowing through the load. Maximize it with respect to $Z_L$ (i.e. $dP/dZ_L=0$).  What value of $Z_L$ will maximize the power delivered to it? 
